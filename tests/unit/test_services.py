...
def test_error_for_invalid_sku():
    ...
    with pytest.raises(services.InvalidSku, match="Invalid sku NONEXISTSKU"):
        services.allocate("o1", "NONEXISTSKU", 10, uow)
...
def test_commits():
    ...
    # repo, session = FakeRepository([]), FakeSession()
    uow = FakeUnitOfWork()
    services.add_batch("b1", "OMNIOUS-MIRROR", 100, None, uow)

    services.allocate("o1", "OMNIOUS-MIRROR", 10, uow)
...
def test_trying_to_deallocate_unallocated_batch():
    ...
    batch = uow.batches.get(reference="b1")
    assert batch.available_quantity == 90
    services.deallocate("o2", "BLUE-PLINTH", 5, uow)
    assert batch.available_quantity == 90
    services.deallocate("o1", "BLUE-PLINTH", 10, uow)
    assert batch.available_quantity == 100
...
def test_prefers_warehouse_batches_to_shipments():
    ...
    # repo, session = FakeRepository([]), FakeSession()
    uow = FakeUnitOfWork()
    services.add_batch("batch-001", "RETRO-CLOCK", 100, None, uow)
    services.add_batch("batch-002", "RETRO-CLOCK", 100, tomorrow, uow)

    services.allocate("order-001", "RETRO-CLOCK", 10, uow)

    assert uow.batches.get("batch-001").available_quantity == 90
    assert uow.batches.get("batch-002").available_quantity == 100
